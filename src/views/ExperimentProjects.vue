  ...
  <div class="experiment-projects">
    ...
    <el-table :data="projects" style="width: 100%" border>
      ...
      <el-table-column label="操作" v-else>
        <template #default="{ row }">
          <el-button type="primary" size="small" @click="viewGuide(row.guide_path)">进入实验</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import {
  ...
  createProject,
  updateProject,
  deleteProject,      // ← 改成 deleteProject
  saveGuideAPI,
  getStudentIPs,
  distributeProject,
} from '@/api/index.js';
  ...
  setup() {
    ...
    const deleteProject = async (id) => {
      if (isProcessing.value) {
        console.warn('删除操作正在进行中，忽略重复点击');
        return;
      }
      isProcessing.value = true;

      try {
        await ElMessageBox.confirm('确定删除此项目？', '提示', {
        ...
        try {
          ...
          projects.value = projects.value.filter(p => p.id !== id); // 本地先移除
          const response = await deleteProject(id); // 调用后端 API
          console.log('Delete API response:', response);
          ElMessage.success('项目删除成功');
        } catch (e) {
          console.error('Delete project API error:', e.response || e);
          let errorMsg = '删除失败：未知错误';
          if (e.response) {
            if (e.response.status === 401) errorMsg = '删除失败：未授权，请重新登录';
            else if (e.response.status === 403) errorMsg = '删除失败：无管理员权限';
            else if (e.response.status === 404) errorMsg = '删除失败：项目不存在';
            else errorMsg = `删除失败：${e.response.data?.error || e.message}`;
          }
        ...
        ElMessage.error('确认操作失败：' + (e.message || '未知错误'));
      } finally {
    ...
    const openDesignModal = (project) => {
      currentProjectId.value = project.id
      // 回显历史步骤，没有就空数组
      const oldSteps = project.guide_steps ? JSON.parse(project.guide_steps) : []
      guideForm.value = {
        project_id: project.id,
        steps: oldSteps.length ? oldSteps : [{ title: '', content: '' }]
      }
    ...
    const distributeProject = async () => {
      if (!distributeFormRef.value) {
        console.error('distributeFormRef is not defined');
        return;
      }
      try {
        await distributeFormRef.value.validate();
        await distributeProject(currentProjectId.value, distributeForm.value);
        ElMessage.success('项目下发成功');
        showDistributeModal.value = false;
        distributeFormRef.value.resetFields();
      } catch (error) {
        console.error('Distribute project error:', error.response || error);
        ElMessage.error('下发项目失败: ' + (error.message || '未知错误'));
      }
    };

    // 查看指导书
    const viewGuide = (guidePath) => {
      if (guidePath) {
        window.open(`/api/guides/${guidePath}`, '_blank');
      } else {
        ElMessage.error('指导书未创建');
      }
    };
